...
const layoutId = "3080264545"; // Vyntra Layout v2
...
export async function askToApplySteamLayout(win: BrowserWindow) {
    ...
    const { response } = await dialog.showMessageBox(win, {
        message: `${update ? "Update" : "Apply"} Vyntra Steam Input Layout?`,
        detail: `Would you like to ${update ? "Update" : "Apply"} Vyntra's recommended Steam Deck controller settings?
